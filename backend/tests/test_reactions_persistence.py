"""Tests for save_reactions + orphan cleanup (Plan 10 D-18 amended, D-19, D-21, D-23).

Covers:
- save_reactions column persistence + JSONB components shape
- long_rinchi_key UNIQUE dedup (D-18 amended)
- NO_RINCHI_{sha1(reaction_smiles)} synthetic fallback
- Extraction.reaction_count update path (D-16)
- delete_extraction_by_id orphan-reaction cleanup (D-21)
...
async def test_save_reactions_deduplicates_by_long_rinchi_key(db_session):
    """D-18 amended: two identical reactions collapse to one Reaction row."""
...
async def test_get_extraction_reactions_ordered_by_position(db_session):
    """D-23: cached reactions returned in insertion order (position asc)."""
...
async def test_get_extraction_reactions_returns_none_for_unknown_id(db_session):
    """D-23: unknown extraction_id -> None (router converts to 404)."""
