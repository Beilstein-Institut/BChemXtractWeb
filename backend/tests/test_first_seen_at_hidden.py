"""PRIV-13: assert first_seen_at is absent from API response shapes.

D-22: the columns ``substances.first_seen_at`` and ``reactions.first_seen_at``
...
JSON shape lives in the existing ``test_extract_endpoint.py`` — see
the PRIV-13 assertion there. We do not add a duplicate HTTP test here
because it would require the JVM + a live DB; the unit assertions
below are sufficient to catch a regression that ever adds the field
...
def test_substance_response_does_not_expose_first_seen_at() -> None:
    """PRIV-13: SubstanceResponse must not declare first_seen_at."""
...
def test_reaction_response_does_not_expose_first_seen_at() -> None:
    """PRIV-13: ReactionResponse must not declare first_seen_at."""
...
def test_substance_orm_column_first_seen_at_retained() -> None:
    """PRIV-13: the DB column stays — D-22 hides the field from the API
    surface but keeps it for ops / forensics.
    """
...
def test_reaction_orm_column_first_seen_at_retained() -> None:
    """PRIV-13: same as above for Reaction."""
