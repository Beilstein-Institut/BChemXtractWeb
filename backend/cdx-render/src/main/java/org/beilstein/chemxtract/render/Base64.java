 ...
 * href="http://iharder.net/base64">http://iharder.net/base64</a>. </p> <p> Change Log: </p> <ul>
 * <li>v2.2.1 - Fixed bug using URL_SAFE and ORDERED encodings. Fixed bug when using very small
 * files (~< 40 bytes).</li> <li>v2.2 - Added some helper methods for encoding/decoding directly
 * from one file to the next. Also added a main() method to support command line encoding/decoding
 * from one file to the next. Also added these Base64 dialects: <ol> <li>The default is RFC3548
 * format.</li> <li>Calling Base64.setFormat(Base64.BASE64_FORMAT.URLSAFE_FORMAT) generates URL
 * and file name friendly format as described in Section 4 of RFC3548.
 ...
 * for contributing the new Base64 dialects. </li> <li>v2.1 - Cleaned up javadoc comments and
 ...
 * with other encodings (like EBCDIC).</li> <li>v2.0.1 - Fixed an error when decoding a single
 * byte, that is, when the encoded data was a single byte.</li> <li>v2.0 - I got rid of methods
 * that used booleans to set options. Now everything is more consolidated and cleaner. The code now
 * detects when data that's being decoded is gzip-compressed and will decompress it automatically.
 * Generally things are cleaner. You'll probably have to change some method calls that you were
 * making to support the new options format (<tt>int</tt>s that you "OR" together).</li> <li>v1.5.1 -
 * Fixed bug when decompressing and decoding to a byte[] using
 ...
 * an otherwise "normal" stream (like an XML file).</li> <li>v1.5 - Output stream pases on flush()
 * command but doesn't do anything itself. This helps when using GZIP streams. Added the ability to
 * GZip-compress objects before encoding them.</li> <li>v1.4 - Added helper methods to read/write
 * files.</li> <li>v1.3.6 - Fixed OutputStream.flush() so that 'position' is reset.</li> <li>v1.3.5 -
 * Added flag to turn on and off line breaks. Fixed bug in input stream where last buffer being
 * read, if not completely full, was not returned.</li> <li>v1.3.4 - Fixed when "improperly padded
 * stream" error was thrown at the wrong time.</li> <li>v1.3.3 - Fixed I/O streams which were
 * totally messed up.</li> </ul> <p> I am placing this code in the Public Domain. Do with it as you
 * will. This software comes with no guarantees or warranties but with plenty of well-wishing
 * instead! Please visit <a href="http://iharder.net/base64">http://iharder.net/base64</a>
...
/* ********  E N C O D I N G   M E T H O D S  ******** */
   ...
   * given by <var>numSigBytes</var>. </p> <p> This is the lowest level of the encoding methods
  ...
  private static byte[] encode3to4(byte[] source, int srcOffset, int numSigBytes, byte[] destination, int destOffset, int options) {
    ...
    int inBuff = (numSigBytes > 0 ? ((source[srcOffset] << 24) >>> 8) : 0) |
            (numSigBytes > 1 ? ((source[srcOffset + 1] << 24) >>> 16) : 0) | (numSigBytes > 2 ? ((source[srcOffset + 2] << 24) >>> 24) : 0);
  ...
  public static String encodeBytes(byte[] source, int off, int len, int options) {
    ...
    if (gzip == GZIP) {
      ...
      try {
        ...
        b64os = new Base64.OutputStream(baos, ENCODE_FLAG | options);
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
   ...
   * does not check to make sure your arrays are large enough to accomodate <var>srcOffset</var> +
  ...
  private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
    ...
    // Example: DkLE
      ...
      try {
        ...
        logger.warn("" + source[srcOffset] + ": " + (DECODABET[source[srcOffset]]));
        logger.warn("" + source[srcOffset + 1] + ": " + (DECODABET[source[srcOffset + 1]]));
        logger.warn("" + source[srcOffset + 2] + ": " + (DECODABET[source[srcOffset + 2]]));
        logger.warn("" + source[srcOffset + 3] + ": " + (DECODABET[source[srcOffset + 3]]));
        return -1;
      } // end catch
    }
  } // end decodeToBytes
  ...
  public static class InputStream extends java.io.FilterInputStream {
     ...
     * @param in the <tt>java.io.InputStream</tt> from which to read data.
     * @param options Specified options
     * @see Base64#ENCODE_FLAG
     * @see Base64#DECODE_FLAG
     * @see Base64#DONT_BREAK_LINES
     * @since 2.0
     */
    public InputStream(java.io.InputStream in, int options) {
      ...
      encode = (options & ENCODE_FLAG) == ENCODE_FLAG;
