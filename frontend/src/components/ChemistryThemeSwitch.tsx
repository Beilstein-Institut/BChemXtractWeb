 ...
 *
 * A glass round-bottom flask with liquid inside slides across a sky
 * track: the day side shows a sun and layered clouds (teal liquid);
 * the night side shows a moon with craters over a starfield (navy
 * liquid). Active-click tips the neck 18° as a "pour"
 * micro-interaction.
 *
 * Design ported from Kohulan/ChemAudit's ChemThemeToggle. Styled
 * components was replaced with plain CSS scoped under `.chem-toggle`
 * in `src/index.css`; the theme wire-up uses the existing
 * ThemeProvider at `src/components/theme-provider.tsx`. When
 * `theme === "system"` the toggle mirrors the current OS preference
 * via `matchMedia`.
 *
 * Emits `data-slot="theme-switch"` so existing tests/specs that
 * target `[data-slot="theme-switch"]` (AppHeader, visual-overhaul,
 * phase3-smoke) continue to resolve it.
 */
