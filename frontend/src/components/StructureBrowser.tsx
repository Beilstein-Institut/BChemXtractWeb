...
export function StructureBrowser({
  ...
  const allSelected =
    substances.length > 0 && substances.every((s) => selectedIds.has(s.id));
  ...
  return (
    <div>
      ...
      {browseState === "success" && substances.length === 0 && (
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <p className="text-body text-muted-foreground max-w-[400px]">
            This extraction contains no structures. Try uploading a different
            file.
          </p>
        </div>
      )}
      ...
      <StructureSheet
        ...
        onNext={() =>
          setSheetIndex((i) => Math.min(substances.length - 1, i + 1))
        }
      />
    </div>
  );
