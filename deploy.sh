...
if compgen -G "backend/jars/bchemxtract-*-jar-with-dependencies.jar" >/dev/null; then
  ...
  # JDK 25+ and Maven are only required when the upstream JAR has to be built.
...
maven not found — needed to build the BChemXtract JAR. Install one of:
    ...
    RHEL/Fedora:    sudo dnf install -y maven java-25-openjdk-devel
    macOS (brew):   brew install maven openjdk@25
MSG
)"
  command -v javac >/dev/null \
    || die "javac not found — Java 25+ JDK required (a JRE is not enough)"
  JAVA_MAJOR=$(java -version 2>&1 | awk -F'"' '/version/ {print $2}' | awk -F. '{print $1}')
  [[ "${JAVA_MAJOR:-0}" =~ ^[0-9]+$ && "${JAVA_MAJOR:-0}" -ge 25 ]] \
    || die "Java 25+ required (found: ${JAVA_MAJOR:-unknown}). The runtime container ships JDK 25; please match it on the host."
